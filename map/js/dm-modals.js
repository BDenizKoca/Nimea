    ...
    class DmModals {
        ...
        setupMarkerCreationModal() {
            ...
            const addImageRow = (val = '') => {
                ...
                row.innerHTML = `<input type="url" placeholder="https://... veya images/ornek.jpg" value="${val}"><button type="button" class="remove-image">Kaldır</button>`;
        ...
        editMarker(markerData) {
            ...
            if (imagesListEl) {
                ...
                images.forEach(url => {
                    const row = document.createElement('div');
                    row.className = 'image-list-item';
                    row.innerHTML = `<input type="url" placeholder="https://... veya images/ornek.jpg" value="${url}"><button type="button" class="remove-image">Kaldır</button>`;
                    row.querySelector('.remove-image').addEventListener('click', () => row.remove());
                    imagesListEl.appendChild(row);
                });
            }
        ...
        saveMarkerFromForm() {
            ...
            const imagesListEl = document.getElementById('marker-images-list');
            if (imagesListEl) {
                const urls = Array.from(imagesListEl.querySelectorAll('input[type="url"]'))
